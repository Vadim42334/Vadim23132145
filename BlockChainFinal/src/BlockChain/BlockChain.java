...
public class BlockChain {
		...
		public static void initBlockChain() throws IOException , InvalidKeyException, NoSuchAlgorithmException, SignatureException
    	{
         ...
         signer.setBalance(13333);										 					// устанавливаем баланс пользователя	
         byte[] sign = SignerUser.signData(message.getBytes());	 
																    
         PublicKey key1 = signer.getPublicKey();

    	 operationData op5 = Operation.createOperation(signer, signer, 20, sign, message);

    	 
    	 
    	 ArrayList <operationData> transactionsData = new ArrayList <operationData>();          // создаем массив транзакций							
    	 transactionsData = Transaction.addToTransaction(op5, transactionsData);					// заносим операции в транзакцию
    	...
    	{		
				...
				output.coinDatabase.put(str_key, signer.getBalance());
    	}
			...
			int txSize = output.BlockChainHistory.get(i).transactionData.size();
				for(int a = 0; a< txSize; a++)
				{
					...
					for(int b = 0; b< opSize; b++)
					{
						...
						{   // Заполняем массив ключами и их балансами
							accounts.add(str_key);
							int transit = output.BlockChainHistory.get(i).transactionData.get(a).operationData.get(b).sender.getBalance();
							accounts.add(Integer.toString(transit));
						...
						{   // Заполняем массив ключами и их балансами
							...
							int transit1 = output.BlockChainHistory.get(i).transactionData.get(a).operationData.get(c).recipient.getBalance();
							accounts.add(Integer.toString(transit1));
    	...
    	{		
			...
			for(int i = 0; i < block.transactionData.size(); i++)
			{
    	...
    	{		
			...
			for(int i = 0; i< blockNumber; i++)
			{
				...
				for(int a = 0; a< txSize; a++)
				{
					output.txDataBase.add(output.BlockChainHistory.get(i).transactionData.get(a));
				}
			}
